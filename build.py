#!/usr/bin/env python3
"""
Boosterpatch Build-Skript
Liest src/template.html + alle CSS/JS-Quelldateien
und erzeugt eine vollständige Single-File boosterpatch.html.

Verwendung:
    python build.py
    python3 build.py

Ausgabe: boosterpatch.html (im selben Verzeichnis, direkt im Browser öffnen)
"""
...
def build():
    ...
    # Ausgabe schreiben
    out = os.path.join(BASE, "boosterpatch.html")
    with open(out, "w", encoding="utf-8") as f:
        f.write(html)

    lines = html.count("\n")
    print(f"✓  boosterpatch.html gebaut ({lines} Zeilen)")
    print(f"   CSS: {', '.join(CSS_FILES)}")
    print(f"   JS:  {', '.join(JS_FILES)}")
    print(f"\n→  Einfach boosterpatch.html im Browser öffnen.")
